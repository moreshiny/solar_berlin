...
class Unet(tensorflow.keras.Model):
    ...
    def _morph_open(self, image, filter_size, n_classes):
    ...
    '''
        filter_ = tensorflow.ones([filter_size, filter_size, n_classes], dtype=image.dtype)
        strides = [1] * 4
        dilations = [1, 1, 1, 1]

        eroded = tensorflow.nn.erosion2d(image, filter_, strides, 'SAME', 'NHWC', dilations)

        filter_ = tensorflow.ones([filter_size, filter_size, n_classes], dtype=image.dtype)
        strides = [1, 1, 1, 1]
        dilations = [1, 3, 3, 1]
        dilated_1 = tensorflow.nn.dilation2d(eroded, filter_, strides, 'SAME', 'NHWC', dilations)


        filter_ = tensorflow.ones([filter_size + 2, filter_size + 2, n_classes], dtype=image.dtype)
        strides = [1] * 4
        dilations = [1, 3, 3, 1]

        eroded_2 = tensorflow.nn.erosion2d(dilated_1, filter_, strides, 'SAME', 'NHWC', dilations)


        filter_ = tensorflow.ones([filter_size+2, filter_size+2, n_classes], dtype=image.dtype)
        strides = [1, 1, 1, 1]
        dilations = [1, 3, 3, 1]
        dilated_2 = tensorflow.nn.dilation2d(eroded_2, filter_, strides, 'SAME', 'NHWC', dilations)
        return dilated_2
